    ...
    def DessineGrille(self, p, grille ):
        ...
        for y in range(self.dimy):
            for x in range(self.dimx):
                ...
                proba = p[y][x]
                couleur = "#%02x%02x%02x" % (round(255*(proba/maxi)),round(255*(proba/maxi)),round(255*(proba/maxi)))
                if maxi == proba:
                    self.can.create_rectangle ( hgx, hgy, bdx, bdy, fill = couleur, outline = grille[y][x], width=5 )
                else:
                    self.can.create_rectangle ( hgx, hgy, bdx, bdy, fill = couleur )
                self.can.create_text ( (hgx+bdx)/2, (hgy+bdy)/2, text = str(round(proba,3)), fill = grille[y][x] )
